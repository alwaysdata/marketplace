...
wget -O- https://sourceforge.net/projects/phplist/files/phplist/3.6.13/phplist-3.6.13.tgz | tar -xz --strip-components=0

sed -i "s|database_host = 'localhost';|database_host = '$DATABASE_HOST';|" phplist-3.6.13/public_html/lists/config/config.php
...
if [ "$INSTALL_URL_PATH" = "/" ]
then
sed -i "s|\$pageroot = '/';|\$pageroot = '';|" phplist-3.6.13/public_html/lists/config/config.php
fi

ADMIN_PASSWORD="$FORM_ADMIN_PASSWORD" ADMIN_EMAIL="$FORM_ADMIN_EMAIL" php phplist-3.6.13/public_html/lists/admin/index.php -p initialise -c $INSTALL_PATH/phplist-3.6.13/public_html/lists/config/config.php

shopt -s dotglob
mv phplist-3.6.13/public_html/lists/* .
rm -rf  .wget-hsts phplist-3.6.13
